...
2. Fonts: Use detected fonts if found. Otherwise suggest fonts that match the brand personality — and phrase them as suggestions ("Suggested:"), never as detected facts.
...
5. GROUNDING — this is the most important rule. Every factual claim must be traceable to the SITE DATA above. NEVER invent founding dates, years in business, statistics, customer counts, awards, certifications, client or partner names, office locations, team size, pricing, or product/service names that do not appear in the scraped text. If the scraped data doesn't cover something, write around it in general terms or omit it — do not fill the gap with plausible-sounding specifics.
6. If the scraped data is thin (few headings/paragraphs), keep the guide shorter and more conservative rather than padding it with invented detail.
...

{
  ...
  "fullGuide": "Brand guide (up to 600 words — shorter if the scraped data is thin) with sections: BRAND STORY, TARGET AUDIENCE, VISUAL IDENTITY, VOICE AND TONE, BRAND PILLARS, DOS AND DONTS. Every claim must come from the scraped site content; no invented history, numbers, names, or achievements."
}`;
      ...
      const sectionPrompts = {
        ...
        fullGuide: `Rewrite the full brand guide for ${currentBrand.brandName}. Use all available site data. Tagline: "${currentBrand.tagline}". Description: "${currentBrand.description}". Voice traits: ${currentBrand.voiceAndTone?.traits?.join(', ')}. H1s: "${siteData.h1s?.join(' | ')}". Paragraphs: "${siteData.paragraphs?.slice(0,3).join(' || ')}". Write a guide of up to 600 words. GROUNDING: every factual claim must come from the data above — never invent founding dates, statistics, awards, client names, locations, team size, or product names that aren't in the scraped text; if data is thin, write a shorter, more conservative guide instead of padding it. Respond ONLY with JSON: {"fullGuide":"..."}`,
      };
