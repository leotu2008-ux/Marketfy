exports.handler = async function (event) {
  ...
  try {
    ...
    if (body.action === "scrape") {
      ...
      // Fetch up to 5 pages (home + linked internal pages)
      ...
      const parseHtml = (html, url) => {
        ...
        // Colors — from style blocks, inline styles, CSS variables
        ...
        hexColors.forEach(c => {
          ...
          const lum = (r + g + b) / 3;
          if (lum > 20 && lum < 240) {
            siteData.colors[c] = (siteData.colors[c] || 0) + 1;
          }
        });

        // RGB colors
        [...html.matchAll(/rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/g)].forEach(m => {
          const r = parseInt(m[1]), g = parseInt(m[2]), b = parseInt(m[3]);
          const lum = (r + g + b) / 3;
          if (lum > 20 && lum < 240) {
            const hex = '#' + [r,g,b].map(x=>x.toString(16).padStart(2,'0')).join('');
            siteData.colors[hex] = (siteData.colors[hex] || 0) + 1;
          }
        });

        // Fonts
        [...html.matchAll(/font-family\s*:\s*([^;}"'{]+)/gi)].forEach(m => {
          const f = m[1].trim().split(',')[0].replace(/['"]/g,'').trim();
          if (f && !siteData.fonts.includes(f) && f.length < 40) siteData.fonts.push(f);
        });
        // Google Fonts
        [...html.matchAll(/fonts\.googleapis\.com\/css[^"']*family=([^&"'|]+)/g)].forEach(m => {
        ...
        [...html.matchAll(/href=["']([^"'#?]+)["']/g)].forEach(m => {
          try {
            ...
            if (u.hostname === base.hostname && !crawledUrls.includes(u.href) && crawledUrls.length < 6) {
              // Only crawl likely content pages
      ...
      // Crawl up to 4 more pages in parallel
      const additionalUrls = crawledUrls.slice(1, 5);
      const additionalPages = await Promise.all(additionalUrls.map(u => fetchPage(u)));
      additionalPages.forEach((html, i) => parseHtml(html, additionalUrls[i]));

      // Sort colors by frequency, deduplicate similar colors
      const sortedColors = Object.entries(siteData.colors)
        .sort((a, b) => b[1] - a[1])
        .map(([hex]) => hex)
        .slice(0, 15);
